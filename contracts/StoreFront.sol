...
contract StoreFront is Initializable, AccessControlUpgradeable, IERC721ReceiverUpgradeable {
	...
	event ItemBurned(address indexed user, address indexed token, uint256 indexed tokenId);
	...
	function burnItem(address user, uint256 tokenId) external {
		...
		require(nftToken.ownerOf(tokenId) == user, "User must be owner of token");
